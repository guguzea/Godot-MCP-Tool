...
static func parse_typed_value(value: Variant, type_hint: int) -> Variant:
	...
	if value is Dictionary:
		...
		match type_hint:
			TYPE_VECTOR2: return Vector2(float(dict.get(&"x", 0)), float(dict.get(&"y", 0)))
			TYPE_VECTOR2I: return Vector2i(int(dict.get(&"x", 0)), int(dict.get(&"y", 0)))
			TYPE_VECTOR3: return Vector3(float(dict.get(&"x", 0)), float(dict.get(&"y", 0)), float(dict.get(&"z", 0)))
			TYPE_VECTOR3I: return Vector3i(int(dict.get(&"x", 0)), int(dict.get(&"y", 0)), int(dict.get(&"z", 0)))
			TYPE_COLOR: return Color(float(dict.get(&"r", 1)), float(dict.get(&"g", 1)), float(dict.get(&"b", 1)), float(dict.get(&"a", 1)))
			TYPE_RECT2: return Rect2(float(dict.get(&"x", 0)), float(dict.get(&"y", 0)), float(dict.get(&"width", 0)), float(dict.get(&"height", 0)))
			TYPE_QUATERNION: return _parse_quaternion_dictionary(dict)
			TYPE_BASIS: return _parse_basis_dictionary(dict)
			TYPE_TRANSFORM3D: return _parse_transform3d_dictionary(dict)
			TYPE_AABB: return _parse_aabb_dictionary(dict)
	return parse_value(value)
...
static func serialize_value(value: Variant) -> Variant:
	match typeof(value):
		...
		TYPE_QUATERNION: return {&"type": &"Quaternion", &"x": value.x, &"y": value.y, &"z": value.z, &"w": value.w}
		TYPE_BASIS: return {&"type": &"Basis", &"x": serialize_value(value.x), &"y": serialize_value(value.y), &"z": serialize_value(value.z)}
		TYPE_TRANSFORM3D: return {&"type": &"Transform3D", &"basis": serialize_value(value.basis), &"origin": serialize_value(value.origin)}
		TYPE_AABB: return {&"type": &"AABB", &"position": serialize_value(value.position), &"size": serialize_value(value.size)}
		TYPE_ARRAY:
			var out: Array = []
			for item in value:
...
static func _parse_explicit_dictionary(type_name: String, dict: Dictionary) -> Variant:
	match type_name:
		...
		"Color": return Color(float(dict.get(&"r", 1)), float(dict.get(&"g", 1)), float(dict.get(&"b", 1)), float(dict.get(&"a", 1)))
		...
		"Rect2": return Rect2(float(dict.get(&"x", 0)), float(dict.get(&"y", 0)), float(dict.get(&"width", 0)), float(dict.get(&"height", 0)))
		"Quaternion": return _parse_quaternion_dictionary(dict)
		"Basis": return _parse_basis_dictionary(dict)
		"Transform3D": return _parse_transform3d_dictionary(dict)
		"AABB": return _parse_aabb_dictionary(dict)
		_: return dict
