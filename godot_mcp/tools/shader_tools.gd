...
"""
		_:
			return _error("Invalid shader type: {}" .format([type]))

	# Write the shader file
	var file = FileAccess.open(path, FileAccess.WRITE)
	if not file:
		return _error("Failed to create shader file: {}" .format([path]))
...
func _read_shader(path: String) -> Dictionary:
	...
	if not file:
		return _error("Failed to read shader: {}" .format([path]))
...
func _write_shader(path: String, code: String) -> Dictionary:
	...
	var file = FileAccess.open(path, FileAccess.WRITE)
	if not file:
		return _error("Failed to write shader: {}" .format([path]))
...
func _get_shader_info(path: String) -> Dictionary:
	...
	if not shader:
		return _error("Shader not found: {}" .format([path]))
...
func _set_uniform_default(path: String, uniform_name: String, value) -> Dictionary:
	...
	if not result:
		...
		if result:
			...
			return _error("Uniform not found: {}" .format([uniform_name]))
	else:
...
func _create_shader_material(args: Dictionary) -> Dictionary:
	...
	if not shader_path.is_empty():
		...
		if not shader:
			return _error("Shader not found: {}" .format([shader_path]))
...
func _get_shader_material_info(path: String) -> Dictionary:
	...
	if not material:
		return _error("ShaderMaterial not found: {}" .format([path]))
...
func _assign_shader_material(material_path: String, node_path: String, surface: int) -> Dictionary:
	...
	if not material:
		return _error("ShaderMaterial not found: {}" .format([material_path]))
	...
	if node is GeometryInstance3D:
		...
		}, "Material assigned to surface {}" .format([surface]))
