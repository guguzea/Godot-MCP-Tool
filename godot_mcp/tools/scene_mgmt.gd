...
func _execute_run(args: Dictionary) -> Dictionary:
	...
	match action:
		...
		"play_custom":
			...
			return _success(null, "Playing scene: {}" .format([path]))
		"stop":
...
func _open_scene(path: String) -> Dictionary:
	...
	if not FileAccess.file_exists(path):
		return _error("Scene file not found: {}" .format([path]))
	...
	return _success({"path": path}, "Scene opened: {}" .format([path]))
...
func _save_scene_as(path: String) -> Dictionary:
	...
	return _success({"path": path}, "Scene saved as: {}" .format([path]))
...
func _create_scene(root_type: String, scene_name: String) -> Dictionary:
	...
	var temp_path = "res://{}.tscn".format([scene_name.to_lower()]).replace(" ", "_")
	...
	return _success({
		...
		"root_type": root_type,
		"name": scene_name
	}, "Scene created: {}" .format([temp_path]))
...
func _reload_scene() -> Dictionary:
	...
	return _success({"path": path}, "Scene reloaded: {}" .format([path]))
...
func _select_nodes(paths: Array) -> Dictionary:
	...
	for path in paths:
		...
		if node:
			...
			errors.append("Node not found: {}" .format([path]))
	...
	return _success(result, "Selected {} nodes" .format([selected_count]))
