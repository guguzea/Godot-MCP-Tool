...
func _remove_from_group(path: String, group: String) -> Dictionary:
	...
	if not node.is_in_group(group):
		return _error("Node is not in group: {}" .format([group]))
...
func _call_group(group: String, method: String, args: Array) -> Dictionary:
	...
	if nodes.is_empty():
		return _error("No nodes in group: {}" .format([group]))
	...
	}, "Method called on {} nodes" .format([called_count]))
...
func _set_group(group: String, property: String, value) -> Dictionary:
	...
	return _success({
		...
		"nodes_count": nodes.size(),
		"set_count": set_count
	}, "Property set on {} nodes" .format([set_count]))
