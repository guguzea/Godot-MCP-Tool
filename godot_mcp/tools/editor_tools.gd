...
func _set_main_screen(screen: String) -> Dictionary:
	...
	return _success({"screen": screen}, "Switched to {} editor" .format([screen]))
...
func _get_editor_setting(setting: String) -> Dictionary:
	...
	if not editor_settings.has_setting(setting):
		return _error("Setting not found: {}" .format([setting]))
...
func _inspect_resource(ei: EditorInterface, resource_path: String) -> Dictionary:
	...
	if not ResourceLoader.exists(resource_path):
		return _error("Resource not found: {}" .format([resource_path]))

	var resource = load(resource_path)
	if not resource:
		return _error("Failed to load resource: {}" .format([resource_path]))
	...
	if dir:
		...
		while not folder.is_empty():
			if dir.current_is_dir() and not folder.begins_with("."):
				var plugin_cfg = "res://addons/{}/plugin.cfg" .format([folder])
...
func _enable_plugin(ei: EditorInterface, plugin_name: String) -> Dictionary:
	...
	var plugin_cfg = "res://addons/{}/plugin.cfg" .format([plugin_name])
	if not FileAccess.file_exists(plugin_cfg):
		return _error("Plugin not found: {}" .format([plugin_name]))
