...
func _create_physics_body(args: Dictionary) -> Dictionary:
	...
	match body_type:
		...
		"area_3d":
			body = Area3D.new()
		_:
			return _error("Unknown body type: {}" .format([body_type]))
...
func _set_body_mode(path: String, mode: String) -> Dictionary:
	...
	match mode:
		...
		"rigid", "rigid_linear":
			node.freeze = false
		_:
			return _error("Unknown mode: {}" .format([mode]))
...
func _create_joint(args: Dictionary) -> Dictionary:
	...
	match joint_type:
		...
		_:
			return _error("Unknown joint type: {}" .format([joint_type]))
...
func _set_joint_param(path: String, param: String, value) -> Dictionary:
	...
	if not prop_name in node:
		return _error("Parameter not found: {}" .format([param]))
