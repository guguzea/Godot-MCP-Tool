...
func modify_node_property(args: Dictionary) -> Dictionary:
	...
	if old_value is Resource and not (parsed is Resource):
		root.queue_free()
		return {&"ok": false, &"error": "Property '{}' expects a Resource. Use specialized tools (set_collision_shape, set_sprite_texture, set_mesh, set_material) instead." .format([property_name])}
...
func detach_script(args: Dictionary) -> Dictionary:
	...
	return {&"ok": true, &"message": "Detached script from node '{}'" .format([node_path])}
	...
	if mesh_type == "file":
		...
		if not (instance is PrimitiveMesh):
			if instance is Node:
				instance.queue_free()
			root.queue_free()
			return {&"ok": false, &"error": "'{}' is not a PrimitiveMesh type" .format([mesh_type])}
		mesh = instance
...
func set_material(args: Dictionary) -> Dictionary:
	...
	if material_type == "file":
		...
		root.queue_free()
		return {&"ok": false, &"error": "Unknown material type: '{}'. Use 'StandardMaterial3D' or 'file'." .format([material_type])}

	var apply_mode: String
	if target is MeshInstance3D:
		if surface_index >= 0:
			target.set_surface_override_material(surface_index, material)
			apply_mode = "surface_override_material[{}]" .format([surface_index])
		else:
