...
func _list_resources(path: String, type_filter: String, recursive: bool) -> Dictionary:
	...
	if not dir:
		return _error("Directory not found: {}" .format([path]))
...
func _get_resource_info(path: String) -> Dictionary:
	...
	if not ResourceLoader.exists(path):
		return _error("Resource not found: {}" .format([path]))

	var resource = load(path)
	if not resource:
		return _error("Failed to load resource: {}" .format([path]))
	...
	match type_name:
		...
		_:
			...
			if ClassDB.class_exists(type_name) and ClassDB.is_parent_class(type_name, "Resource"):
				...
				return _error("Unknown resource type: {}" .format([type_name]))

	if not resource:
		return _error("Failed to create resource of type: {}" .format([type_name]))
...
func _copy_resource(source: String, dest: String) -> Dictionary:
	...
	if not FileAccess.file_exists(source):
		return _error("Source not found: {}" .format([source]))
...
func _reload_resource(path: String) -> Dictionary:
	...
	if not resource:
		return _error("Failed to reload resource: {}" .format([path]))
...
func _get_texture_info(path: String) -> Dictionary:
	...
	if not ResourceLoader.exists(path):
		return _error("Texture not found: {}" .format([path]))

	var texture = load(path) as Texture2D
	if not texture:
		return _error("Not a valid texture: {}" .format([path]))
...
func _assign_texture_to_node(texture_path: String, node_path: String, property: String) -> Dictionary:
	...
	if not texture:
		return _error("Failed to load texture: {}" .format([texture_path]))

	var node = _find_node_by_path(node_path)
	if not node:
		return _error("Node not found: {}" .format([node_path]))

	if not property in node:
		return _error("Property '{}' not found on node" .format([property]))
