...
func configure_input_map(args: Dictionary) -> Dictionary:
	...
	match operation:
		...
		_:
			return {&"ok": false, &"error": "Unknown operation: {}. Use: add, remove, set" .format([operation])}
...
func _create_input_event(desc: Dictionary) -> Dictionary:
	...
	match type:
		...
		_:
			return {&"error": "Unknown event type: '{}'. Use: key, mouse_button, joypad_button, joypad_motion" .format([type])}
...
func setup_autoload(args: Dictionary) -> Dictionary:
	...
	match operation:
		...
		_:
			return {&"ok": false, &"error": "Unknown operation: {}. Use: add, remove, list" .format([operation])}
...
func open_in_godot(args: Dictionary) -> Dictionary:
	...
	return {&"ok": true, &"message": "Opened %s%s" % [path, " at line {}" .format([line]) if line > 0 else ""]}
...
func _dump_node(node: Node, depth: int) -> String:
	...
	if script:
		line += " [{}]" % [script.resource_path.get_file()]
...
func run_scene(args: Dictionary) -> Dictionary:
	...
	if scene == "current":
		...
		if not FileAccess.file_exists(scene):
			return {&"ok": false, &"error": "Scene file not found: {}" .format([scene])}
		ei.play_custom_scene(scene)
	else:
		ei.play_main_scene()

	return {&"ok": true, &"message": "Scene launched" + (" ({})" .format([scene]) if not scene.is_empty() else " (main scene)")}
...
func classdb_query(args: Dictionary) -> Dictionary:
	...
	if not ClassDB.class_exists(class_name_str):
		return {&"ok": false, &"error": "Class '{}' does not exist in ClassDB" .format([class_name_str])}
