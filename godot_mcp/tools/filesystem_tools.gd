...
func _list_directory(path: String) -> Dictionary:
	...
	if not dir:
		return _error("Cannot open directory: {}" .format([path]))
...
func _read_file(path: String) -> Dictionary:
	...
	if not file:
		return _error("Cannot read file: {}" .format([path]))
...
func _write_file(path: String, content: String) -> Dictionary:
	...
	if not file:
		return _error("Cannot write file: {}" .format([path]))
...
func _delete_file(path: String) -> Dictionary:
	...
	if not FileAccess.file_exists(path):
		return _error("File not found: {}" .format([path]))
...
func _copy_file(source: String, dest: String) -> Dictionary:
	...
	if not FileAccess.file_exists(source):
		return _error("Source file not found: {}" .format([source]))
	...
	for k in keys:
		if data is Dictionary and data.has(k):
			...
			if index >= 0 and index < data.size():
				...
				return _error("Array index out of bounds: {}" .format([key]))
		else:
			return _error("Key not found: {}" .format([key]))
