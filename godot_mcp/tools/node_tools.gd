...
CONNECTION FLAGS: deferred, persist, one_shot
...
- Propagate: {"action": "propagate_call", "path": "/root/Enemies", "method": "reset"}""",
			"inputSchema": {
				"type": "object",
				"properties": {
...
- set_collision_layer: Set collision layer
...
func _execute_query(args: Dictionary) -> Dictionary:
	...
	match action:
		"find_by_name": return _find_by_name(args.get("pattern", ""))
		"find_by_type": return _find_by_type(args.get("type", ""))
		"find_children": return _find_children(args)
		"find_parent": return _find_parent(args.get("path", ""), args.get("pattern", ""))
		"get_info": return _get_node_info(args.get("path", ""))
		"get_children": return _get_node_children(args.get("path", ""))
		"get_path_to": return _get_path_to(args.get("from_path", ""), args.get("to_path", ""))
		"tree_string": return _get_tree_string(args.get("path", ""))
		_: return _error("Unknown action: {}" .format([action]))

func _find_by_name(pattern: String) -> Dictionary:
	if pattern.is_empty(): return _error("Pattern is required")
	var nodes = _find_nodes_by_name(pattern)
	var results: Array[Dictionary] = []
	for node in nodes: results.append(_node_to_dict(node, false))
	return _success({"count": results.size(), "nodes": results})

func _find_by_type(type_name: String) -> Dictionary:
	if type_name.is_empty(): return _error("Type is required")
...
func _find_children(args: Dictionary) -> Dictionary:
	...
	if not node: return _error("Node not found: {}" .format([path]))
	var found = node.find_children(pattern, type_filter, recursive, owned)
	var results: Array[Dictionary] = []
	for child in found: results.append(_node_to_dict(child, false))
	return _success({"count": results.size(), "nodes": results})

func _find_parent(path: String, pattern: String) -> Dictionary:
	if path.is_empty(): return _error("Path is required")
	if pattern.is_empty(): return _error("Pattern is required")
	var node = _find_node_by_path(path)
	if not node: return _error("Node not found: {}" .format([path]))
	var found = node.find_parent(pattern)
	if found: return _success(_node_to_dict(found, false))
...
func _get_node_children(path: String) -> Dictionary:
	...
	for child in node.get_children(): children.append(_node_to_dict(child, false))
...
func _get_path_to(from_path: String, to_path: String) -> Dictionary:
	...
	if not from_node: return _error("From node not found: {}" .format([from_path]))
	var to_node = _find_node_by_path(to_path)
	if not to_node: return _error("To node not found: {}" .format([to_path]))
	var relative_path = from_node.get_path_to(to_node)
	return _success({"from": from_path, "to": to_path, "relative_path": str(relative_path)})
...
func _build_tree_string(node: Node, depth: int) -> String:
	...
	for child in node.get_children(): result += _build_tree_string(child, depth + 1)
	return result
...
func _execute_lifecycle(args: Dictionary) -> Dictionary:
	var action = args.get("action", "")
	match action:
		"create": return _create_node(args.get("type", "Node"), args.get("name", ""), args.get("parent_path", ""))
		"delete": return _delete_node(args.get("path", ""))
		"duplicate": return _duplicate_node(args.get("path", ""), args.get("new_name", ""), args.get("flags", []))
		"instantiate": return _instantiate_scene(args.get("scene_path", ""), args.get("parent_path", ""), args.get("name", ""))
		"replace": return _replace_node(args.get("path", ""), args.get("new_node_path", ""))
		"request_ready": return _request_ready(args.get("path", ""))
		_: return _error("Unknown action: {}" .format([action]))
...
func _create_node(type_name: String, node_name: String, parent_path: String) -> Dictionary:
	...
	if not parent: return _error("Parent node not found: {}" .format([parent_path]))
	var node = ClassDB.instantiate(type_name)
	if not node: return _error("Failed to create node of type: {}" .format([type_name]))
	if not node_name.is_empty(): node.name = node_name
	parent.add_child(node)
	node.owner = _get_edited_scene_root()
	return _success({"path": _get_scene_path(node), "type": type_name, "name": str(node.name)}, "Node created: %s" % str(node.name))

func _delete_node(path: String) -> Dictionary:
	if path.is_empty(): return _error("Path is required")
	var node = _find_node_by_path(path)
	if not node: return _error("Node not found: {}" .format([path]))
	if node == _get_edited_scene_root(): return _error("Cannot delete scene root node")
	var name = str(node.name)
	node.queue_free()
	return _success({"deleted": path}, "Node deleted: {}" .format([name]))
...
func _duplicate_node(path: String, new_name: String, flags: Array) -> Dictionary:
	...
	if not node: return _error("Node not found: {}" .format([path]))
	var flag_value = 0
	if "signals" in flags or flags.is_empty(): flag_value |= Node.DUPLICATE_SIGNALS
	if "groups" in flags or flags.is_empty(): flag_value |= Node.DUPLICATE_GROUPS
	if "scripts" in flags or flags.is_empty(): flag_value |= Node.DUPLICATE_SCRIPTS
	var duplicated = node.duplicate(flag_value)
	if not duplicated: return _error("Failed to duplicate node")
	if not new_name.is_empty(): duplicated.name = new_name
	var parent = node.get_parent()
	parent.add_child(duplicated)
	duplicated.owner = _get_edited_scene_root()
	return _success({"original": path, "new_path": _get_scene_path(duplicated), "name": str(duplicated.name)}, "Node duplicated")

func _instantiate_scene(scene_path: String, parent_path: String, instance_name: String) -> Dictionary:
	if scene_path.is_empty(): return _error("Scene path is required")
	if not scene_path.begins_with("res://"): scene_path = "res://" + scene_path
	if not ResourceLoader.exists(scene_path): return _error("Scene not found: {}" .format([scene_path]))
	var packed_scene = load(scene_path) as PackedScene
	if not packed_scene: return _error("Failed to load scene: {}" .format([scene_path]))
	var instance = packed_scene.instantiate()
	if not instance: return _error("Failed to instantiate scene")
	if not instance_name.is_empty(): instance.name = instance_name
	var parent = _find_node_by_path(parent_path) if not parent_path.is_empty() else _get_edited_scene_root()
	if not parent: instance.queue_free(); return _error("Parent node not found: {}" .format([parent_path]))
	parent.add_child(instance)
	instance.owner = _get_edited_scene_root()
	return _success({"scene": scene_path, "path": _get_scene_path(instance), "name": str(instance.name)}, "Scene instantiated")

func _replace_node(path: String, new_node_path: String) -> Dictionary:
	if path.is_empty() or new_node_path.is_empty(): return _error("Both paths are required")
	var node = _find_node_by_path(path)
	if not node: return _error("Node not found: {}" .format([path]))
	var new_node = _find_node_by_path(new_node_path)
	if not new_node: return _error("New node not found: {}" .format([new_node_path]))
	node.replace_by(new_node)
	return _success({"replaced": path, "replacement": _get_scene_path(new_node)}, "Node replaced")
...
func _set_position(node: Node, args: Dictionary) -> Dictionary:
	...
	if node is Node2D or node is Control:
		if use_global: node.global_position = Vector2(x, y)
		else: node.position = Vector2(x, y)
	elif node is Node3D:
		if use_global: node.global_position = Vector3(x, y, z)
		else: node.position = Vector3(x, y, z)
	else: return _error("Node does not support position")
	return _success({"path": _get_scene_path(node), "position": _get_position_dict(node), "global": use_global}, "Position set")

func _set_rotation(node: Node, radians: float, args: Dictionary) -> Dictionary:
	var use_global = args.get("global", false)
	if node is Node2D or node is Control:
		if use_global: node.global_rotation = radians
		else: node.rotation = radians
	elif node is Node3D:
		var rx = args.get("x", 0.0)
		var ry = args.get("y", 0.0)
		var rz = args.get("z", 0.0)
		if args.has("x") or args.has("y") or args.has("z"):
			if use_global: node.global_rotation = Vector3(rx, ry, rz)
			else: node.rotation = Vector3(rx, ry, rz)
		else: node.rotation.y = radians
	elif node is Control: node.rotation = radians
	else: return _error("Node does not support rotation")
	return _success({"path": _get_scene_path(node), "rotation": _get_rotation_dict(node)}, "Rotation set")

func _set_scale(node: Node, args: Dictionary) -> Dictionary:
	var x = args.get("x", 1.0)
	var y = args.get("y", 1.0)
	var z = args.get("z", 1.0)
	if node is Node2D or node is Control: node.scale = Vector2(x, y)
	elif node is Node3D: node.scale = Vector3(x, y, z)
	else: return _error("Node does not support scale")
	return _success({"path": _get_scene_path(node), "scale": _get_scale_dict(node)}, "Scale set")
...
func _get_transform(node: Node) -> Dictionary:
	...
	else: return _error("Node does not support transform")
...
func _move_node(node: Node, args: Dictionary) -> Dictionary:
	...
	var y = args.get("y", 0.0)
	var z = args.get("z", 0.0)
	if node is Node2D or node is Control: node.position += Vector2(x, y)
	elif node is Node3D: node.position += Vector3(x, y, z)
	else: return _error("Node does not support position")
	return _success({"path": _get_scene_path(node), "new_position": _get_position_dict(node)}, "Node moved")

func _rotate_node(node: Node, args: Dictionary) -> Dictionary:
	var radians = args.get("radians", 0.0)
	if args.has("degrees"): radians = deg_to_rad(args.get("degrees", 0.0))
	if node is Node2D or node is Control: node.rotation += radians
	elif node is Node3D: node.rotation.y += radians
	else: return _error("Node does not support rotation")
	return _success({"path": _get_scene_path(node), "new_rotation": _get_rotation_dict(node)}, "Node rotated")

func _look_at(node: Node, args: Dictionary) -> Dictionary:
	var x = args.get("x", 0.0)
	var y = args.get("y", 0.0)
	var z = args.get("z", 0.0)
	if node is Node2D: node.look_at(Vector2(x, y))
	elif node is Node3D: node.look_at(Vector3(x, y, z))
	else: return _error("Node does not support look_at")
	return _success({"path": _get_scene_path(node), "looking_at": {"x": x, "y": y, "z": z}}, "Node looking at target")

func _reset_transform(node: Node) -> Dictionary:
	if node is Node2D: node.position = Vector2.ZERO; node.rotation = 0; node.scale = Vector2.ONE
	elif node is Node3D: node.position = Vector3.ZERO; node.rotation = Vector3.ZERO; node.scale = Vector3.ONE
	elif node is Control: node.position = Vector2.ZERO; node.rotation = 0; node.scale = Vector2.ONE
	else: return _error("Node does not support transform")
	return _success({"path": _get_scene_path(node)}, "Transform reset")

func _get_position_dict(node: Node) -> Dictionary:
	if node is Node2D or node is Control: return {"x": node.position.x, "y": node.position.y}
	elif node is Node3D: return {"x": node.position.x, "y": node.position.y, "z": node.position.z}
	return {}

func _get_rotation_dict(node: Node) -> Variant:
	if node is Node2D or node is Control: return node.rotation
	elif node is Node3D: return {"x": node.rotation.x, "y": node.rotation.y, "z": node.rotation.z}
	return 0

func _get_scale_dict(node: Node) -> Dictionary:
	if node is Node2D or node is Control: return {"x": node.scale.x, "y": node.scale.y}
	elif node is Node3D: return {"x": node.scale.x, "y": node.scale.y, "z": node.scale.z}
	return {}
...
func _get_property(node: Node, property: String) -> Dictionary:
	if property.is_empty(): return _error("Property name is required")
	if not property in node: return _error("Property not found: {}" .format([property]))
	var value = node.get(property)
	return _success({"property": property, "value": _serialize_value(value), "type": typeof(value), "type_name": _type_to_string(typeof(value))})
...
func _set_property(node: Node, property: String, value) -> Dictionary:
	...
	if not property in node:
		...
		for prop in node.get_property_list():
			...
			if prop.usage & PROPERTY_USAGE_EDITOR and not prop_name.begins_with("_"):
				if property.to_lower() in prop_name.to_lower(): available.append(prop_name)
		var hints = []
		if not available.is_empty(): hints.append("Similar: %s" % ", ".join(available.slice(0, 5)))
		return _error("Property not found: {}" .format([property]), {"node_type": str(node.get_class())}, hints)
	...
	if prop_info.has("valid_values") and value is String:
		...
		for i in range(enum_values.size()):
			...
			if enum_name.to_lower() == value.to_lower(): found_idx = i; break
		if found_idx >= 0: value = found_idx
		else: return _error("Invalid enum value: '{}'" .format([value]), {}, ["Valid values: %s" % ", ".join(enum_values)])
	...
	return _success({"property": property, "old_value": old_value, "new_value": new_value, "node_path": _get_scene_path(node), "node_type": str(node.get_class())}, "Property set: {}" .format([property]))

func _list_properties(node: Node, filter: String) -> Dictionary:
	var properties: Array[Dictionary] = []
	for prop in node.get_property_list():
		var prop_name = str(prop.name)
		if prop_name.begins_with("_") or prop.usage & PROPERTY_USAGE_INTERNAL: continue
		if not (prop.usage & PROPERTY_USAGE_EDITOR): continue
		if not filter.is_empty() and not filter.to_lower() in prop_name.to_lower(): continue
		var prop_data = {"name": prop_name, "type": prop.type, "type_name": _type_to_string(prop.type), "hint": prop.hint, "hint_string": str(prop.hint_string)}
		if prop.type in [TYPE_BOOL, TYPE_INT, TYPE_FLOAT, TYPE_STRING, TYPE_VECTOR2, TYPE_VECTOR3, TYPE_COLOR]: prop_data["current_value"] = _serialize_value(node.get(prop_name))
		if prop.hint == PROPERTY_HINT_ENUM and not prop.hint_string.is_empty(): prop_data["valid_values"] = prop.hint_string.split(",")
		properties.append(prop_data)
	return _success({"path": _get_scene_path(node), "type": str(node.get_class()), "count": properties.size(), "properties": properties})
...
func _reset_property(node: Node, property: String) -> Dictionary:
	...
	if default_instance:
		...
		return _success({"property": property, "value": _serialize_value(node.get(property))}, "Property reset: {}" .format([property]))
...
func _check_revert(node: Node, property: String) -> Dictionary:
	...
	if can_revert: result["revert_value"] = _serialize_value(node.property_get_revert(property))
	return _success(result)
...
func _reparent_node(node: Node, new_parent_path: String, keep_global: bool) -> Dictionary:
	if new_parent_path.is_empty(): return _error("New parent path is required")
	var new_parent = _find_node_by_path(new_parent_path)
	if not new_parent: return _error("New parent not found: {}" .format([new_parent_path]))
	if node == _get_edited_scene_root(): return _error("Cannot reparent scene root")
	var old_parent_path = _get_scene_path(node.get_parent())
	node.reparent(new_parent, keep_global)
	node.owner = _get_edited_scene_root()
	return _success({"path": _get_scene_path(node), "old_parent": old_parent_path, "new_parent": new_parent_path, "keep_global": keep_global}, "Node reparented")

func _reorder_node(node: Node, index: int) -> Dictionary:
	var parent = node.get_parent()
	if not parent: return _error("Node has no parent")
	parent.move_child(node, index)
	return _success({"path": _get_scene_path(node), "new_index": node.get_index()}, "Node reordered")
...
func _move_sibling(node: Node, direction: int) -> Dictionary:
	...
	if new_index < 0 or new_index >= parent.get_child_count(): return _error("Cannot move node further")
	parent.move_child(node, new_index)
	return _success({"path": _get_scene_path(node), "old_index": current_index, "new_index": node.get_index()}, "Node moved")
...
func _set_owner(node: Node, owner_path: String) -> Dictionary:
	...
	if not new_owner: return _error("Owner not found: {}" .format([owner_path]))
...
func _get_owner(node: Node) -> Dictionary:
	var owner = node.owner
	return _success({"path": _get_scene_path(node), "owner": _get_scene_path(owner) if owner else null})
...
func _list_signals(path: String) -> Dictionary:
	...
	for sig in node.get_signal_list():
		...
		for arg in sig.args: sig_info["args"].append({"name": str(arg.name), "type": arg.type, "type_name": _type_to_string(arg.type)})
...
func _has_signal(path: String, signal_name: String) -> Dictionary:
	var node = _find_node_by_path(path)
	if not node: return _error("Node not found: {}" .format([path]))
	return _success({"path": path, "signal_name": signal_name, "exists": node.has_signal(signal_name)})

func _get_signal_connections(path: String, signal_name: String) -> Dictionary:
	if signal_name.is_empty(): return _error("Signal name is required")
	var node = _find_node_by_path(path)
	if not node: return _error("Node not found: {}" .format([path]))
	var connections: Array[Dictionary] = []
	for conn in node.get_signal_connection_list(signal_name):
		var target_obj = conn.callable.get_object()
		connections.append({"signal": str(conn.signal.get_name()), "target": _get_scene_path(target_obj) if target_obj and target_obj is Node else str(target_obj), "method": str(conn.callable.get_method()), "flags": conn.flags})
...
func _get_incoming_connections(path: String) -> Dictionary:
	...
	for conn in node.get_incoming_connections():
		...
		connections.append({"source": _get_scene_path(source_obj) if source_obj and source_obj is Node else str(source_obj), "signal": str(conn.signal.get_name()), "method": str(conn.callable.get_method())})
...
func _connect_signal(args: Dictionary) -> Dictionary:
	...
	if signal_name.is_empty() or method.is_empty(): return _error("Signal name and method are required")
	var source = _find_node_by_path(source_path)
	if not source: return _error("Source node not found: {}" .format([source_path]))
	var target = _find_node_by_path(target_path)
	if not target: return _error("Target node not found: {}" .format([target_path]))
	if not source.has_signal(signal_name): return _error("Signal not found: {}" .format([signal_name]))
	var callable = Callable(target, method)
	if source.is_connected(signal_name, callable): return _error("Signal already connected")
	var flags = 0
	...
	if error != OK: return _error("Failed to connect signal: %s" % error_string(error))
	return _success({"source": source_path, "signal": signal_name, "target": target_path, "method": method, "flags": flags_array}, "Signal connected")
...
func _disconnect_signal(args: Dictionary) -> Dictionary:
	...
	if not target: return _error("Target node not found: {}" .format([target_path]))
	var callable = Callable(target, method)
	if not source.is_connected(signal_name, callable): return _error("Signal not connected")
	source.disconnect(signal_name, callable)
	return _success({"source": source_path, "signal": signal_name, "target": target_path, "method": method}, "Signal disconnected")
...
func _disconnect_all(path: String, signal_name: String) -> Dictionary:
	...
	var count = 0
	for conn in connections: node.disconnect(signal_name, conn.callable); count += 1
	return _success({"path": path, "signal": signal_name, "disconnected_count": count}, "All connections disconnected")
...
func _is_signal_connected(args: Dictionary) -> Dictionary:
	...
	var target = _find_node_by_path(target_path)
	if not target: return _error("Target node not found: {}" .format([target_path]))
	return _success({"source": source_path, "signal": signal_name, "target": target_path, "method": method, "connected": source.is_connected(signal_name, Callable(target, method))})

func _emit_signal(path: String, signal_name: String, args: Array) -> Dictionary:
	if signal_name.is_empty(): return _error("Signal name is required")
	var node = _find_node_by_path(path)
	if not node: return _error("Node not found: {}" .format([path]))
	if not node.has_signal(signal_name): return _error("Signal not found: {}" .format([signal_name]))
	match args.size():
...
func _add_user_signal(args: Dictionary) -> Dictionary:
	...
	var node = _find_node_by_path(path)
	if not node: return _error("Node not found: {}" .format([path]))
	if node.has_signal(signal_name): return _error("Signal already exists: {}" .format([signal_name]))
	var arg_array: Array = []
	for arg in signal_args:
		if arg is Dictionary: arg_array.append({"name": arg.get("name", "arg"), "type": arg.get("type", TYPE_NIL)})
	node.add_user_signal(signal_name, arg_array)
	return _success({"path": path, "signal_name": signal_name, "args": arg_array}, "User signal added")
...
func _add_to_group(path: String, group: String, persistent: bool) -> Dictionary:
	if group.is_empty(): return _error("Group name is required")
	var node = _find_node_by_path(path)
	if not node: return _error("Node not found: {}" .format([path]))
	if node.is_in_group(group): return _error("Node already in group: {}" .format([group]))
...
func _remove_from_group(path: String, group: String) -> Dictionary:
	...
	var node = _find_node_by_path(path)
	if not node: return _error("Node not found: {}" .format([path]))
	if not node.is_in_group(group): return _error("Node not in group: {}" .format([group]))
...
func _get_nodes_in_group(group: String) -> Dictionary:
	...
	if not tree: return _error("Scene tree not available")
	var nodes = tree.get_nodes_in_group(group)
	var result: Array[Dictionary] = []
	for node in nodes: result.append({"name": str(node.name), "path": _get_scene_path(node), "type": str(node.get_class())})
...
func _get_first_in_group(group: String) -> Dictionary:
	...
	if first: return _success(_node_to_dict(first, false))
...
func _count_in_group(group: String) -> Dictionary:
	...
	var tree = root.get_tree()
	if not tree: return _error("Scene tree not available")
	return _success({"group": group, "count": tree.get_nodes_in_group(group).size()})

func _call_group_method(group: String, method: String, args: Array) -> Dictionary:
	if group.is_empty() or method.is_empty(): return _error("Group name and method are required")
	var root = _get_edited_scene_root()
	if not root: return _error("No scene open")
	var tree = root.get_tree()
	if not tree: return _error("Scene tree not available")
...
func _set_group_property(group: String, property: String, value) -> Dictionary:
	if group.is_empty() or property.is_empty(): return _error("Group name and property are required")
	var root = _get_edited_scene_root()
	if not root: return _error("No scene open")
	var tree = root.get_tree()
	if not tree: return _error("Scene tree not available")
...
func _get_process_status(node: Node) -> Dictionary:
	return _success({"path": _get_scene_path(node), "processing": node.is_processing(), "physics_processing": node.is_physics_processing(), "process_mode": node.process_mode, "process_priority": node.process_priority})
...
func _set_process_mode(node: Node, mode: String) -> Dictionary:
	...
	match mode.to_lower():
		...
		_: return _error("Invalid process mode: {}" .format([mode]))
...
func _get_metadata(node: Node, key: String) -> Dictionary:
	if key.is_empty(): return _error("Key is required")
	if not node.has_meta(key): return _error("Metadata not found: {}" .format([key]))
	return _success({"path": _get_scene_path(node), "key": key, "value": _serialize_value(node.get_meta(key))})

func _set_metadata(node: Node, key: String, value) -> Dictionary:
	if key.is_empty(): return _error("Key is required")
	node.set_meta(key, value)
	return _success({"path": _get_scene_path(node), "key": key, "value": _serialize_value(value)}, "Metadata set")

func _has_metadata(node: Node, key: String) -> Dictionary:
	if key.is_empty(): return _error("Key is required")
	return _success({"path": _get_scene_path(node), "key": key, "exists": node.has_meta(key)})
...
func _list_metadata(node: Node) -> Dictionary:
	...
	for key in node.get_meta_list(): keys.append(str(key))
	return _success({"path": _get_scene_path(node), "count": keys.size(), "keys": keys})
...
func _call_method(node: Node, method: String, args: Array) -> Dictionary:
	if method.is_empty(): return _error("Method name is required")
	if not node.has_method(method): return _error("Method not found: {}" .format([method]))
	var result
	match args.size():
		...
		_: return _error("Too many arguments (max 5)")
	return _success({"path": _get_scene_path(node), "method": method, "result": _serialize_value(result)}, "Method called")
...
func _call_method_deferred(node: Node, method: String, args: Array) -> Dictionary:
	...
	match args.size():
		...
		_: return _error("Too many arguments (max 4)")
	return _success({"path": _get_scene_path(node), "method": method, "deferred": true}, "Method call deferred")
...
func _propagate_call(node: Node, method: String, args: Array, parent_first: bool) -> Dictionary:
	...
	return _success({"path": _get_scene_path(node), "method": method, "args_count": args.size(), "parent_first": parent_first}, "Call propagated")

func _has_method(node: Node, method: String) -> Dictionary:
	if method.is_empty(): return _error("Method name is required")
	return _success({"path": _get_scene_path(node), "method": method, "exists": node.has_method(method)})
...
func _get_method_list(node: Node, filter: String) -> Dictionary:
	...
	for method in node.get_method_list():
		...
		if method_name.begins_with("_"): continue
		if not filter.is_empty() and not filter.to_lower() in method_name.to_lower(): continue
		var args_info: Array = []
		for arg in method.args: args_info.append({"name": str(arg.name), "type": _type_to_string(arg.type)})
		methods.append({"name": method_name, "args": args_info, "return_type": _type_to_string(method.return.type)})
	return _success({"path": _get_scene_path(node), "count": methods.size(), "methods": methods})
...
func _set_visible(node: Node, visible: bool) -> Dictionary:
	if node is CanvasItem: node.visible = visible
	elif node is Node3D: node.visible = visible
	else: return _error("Node does not support visibility")
	return _success({"path": _get_scene_path(node), "visible": visible}, "Visibility set")

func _toggle_visible(node: Node) -> Dictionary:
	if node is CanvasItem: node.visible = not node.visible; return _success({"path": _get_scene_path(node), "visible": node.visible}, "Visibility toggled")
	elif node is Node3D: node.visible = not node.visible; return _success({"path": _get_scene_path(node), "visible": node.visible}, "Visibility toggled")
	return _error("Node does not support visibility")

func _is_visible(node: Node) -> Dictionary:
	var visible = false
	var visible_in_tree = false
	if node is CanvasItem: visible = node.visible; visible_in_tree = node.is_visible_in_tree()
	elif node is Node3D: visible = node.visible; visible_in_tree = node.is_visible_in_tree()
	else: return _error("Node does not support visibility")
	return _success({"path": _get_scene_path(node), "visible": visible, "visible_in_tree": visible_in_tree})

func _set_z_index(node: Node, z_index: int) -> Dictionary:
	if not node is CanvasItem: return _error("Node is not a CanvasItem")
...
func _set_modulate(node: Node, color_dict: Dictionary) -> Dictionary:
	...
	node.modulate = color
	return _success({"path": _get_scene_path(node), "modulate": {"r": color.r, "g": color.g, "b": color.b, "a": color.a}}, "Modulate set")

func _set_self_modulate(node: Node, color_dict: Dictionary) -> Dictionary:
	if not node is CanvasItem: return _error("Node is not a CanvasItem")
	var color = Color(color_dict.get("r", 1.0), color_dict.get("g", 1.0), color_dict.get("b", 1.0), color_dict.get("a", 1.0))
	node.self_modulate = color
	return _success({"path": _get_scene_path(node), "self_modulate": {"r": color.r, "g": color.g, "b": color.b, "a": color.a}}, "Self modulate set")

func _set_visibility_layer(node: Node, layer: int) -> Dictionary:
	if node is CanvasItem: node.visibility_layer = layer
	elif node is Node3D: node.layers = layer
	else: return _error("Node does not support visibility layers")
...
func _get_collision_info(node: Node) -> Dictionary:
	...
	if node is CollisionObject2D: info["collision_layer"] = node.collision_layer; info["collision_mask"] = node.collision_mask; info["layers"] = _get_collision_layers(node.collision_layer); info["masks"] = _get_collision_layers(node.collision_mask)
	elif node is CollisionObject3D: info["collision_layer"] = node.collision_layer; info["collision_mask"] = node.collision_mask; info["layers"] = _get_collision_layers(node.collision_layer); info["masks"] = _get_collision_layers(node.collision_mask)
	else: return _error("Node is not a collision object")
	return _success(info)

func _get_collision_layers(bitmask: int) -> Array:
	var layers: Array = []
	for i in range(32):
		if bitmask & (1 << i): layers.append(i + 1)
	return layers

func _set_collision_layer(node: Node, value: int) -> Dictionary:
	if node is CollisionObject2D: node.collision_layer = value
	elif node is CollisionObject3D: node.collision_layer = value
	else: return _error("Node is not a collision object")
...
func _set_collision_mask(node: Node, value: int) -> Dictionary:
	if node is CollisionObject2D: node.collision_mask = value
	elif node is CollisionObject3D: node.collision_mask = value
	else: return _error("Node is not a collision object")
...
func _set_collision_layer_value(node: Node, layer: int, value: bool) -> Dictionary:
	if layer < 1 or layer > 32: return _error("Layer must be between 1 and 32")
	if node is CollisionObject2D: node.set_collision_layer_value(layer, value)
	elif node is CollisionObject3D: node.set_collision_layer_value(layer, value)
	else: return _error("Node is not a collision object")
...
func _set_collision_mask_value(node: Node, layer: int, value: bool) -> Dictionary:
	if layer < 1 or layer > 32: return _error("Layer must be between 1 and 32")
	if node is CollisionObject2D: node.set_collision_mask_value(layer, value)
	elif node is CollisionObject3D: node.set_collision_mask_value(layer, value)
	else: return _error("Node is not a collision object")
...
func _apply_impulse(node: Node, args: Dictionary) -> Dictionary:
	...
	var y = args.get("y", 0.0)
	var z = args.get("z", 0.0)
	if node is RigidBody2D: node.apply_impulse(Vector2(x, y))
	elif node is RigidBody3D: node.apply_impulse(Vector3(x, y, z))
	else: return _error("Node is not a RigidBody")
...
func _apply_force(node: Node, args: Dictionary) -> Dictionary:
	...
	var y = args.get("y", 0.0)
	var z = args.get("z", 0.0)
	if node is RigidBody2D: node.apply_force(Vector2(x, y))
	elif node is RigidBody3D: node.apply_force(Vector3(x, y, z))
	else: return _error("Node is not a RigidBody")
...
func _apply_torque(node: Node, args: Dictionary) -> Dictionary:
	...
	if node is RigidBody2D: node.apply_torque(torque)
	elif node is RigidBody3D: node.apply_torque(Vector3(args.get("x", 0.0), args.get("y", 0.0), args.get("z", 0.0)))
	else: return _error("Node is not a RigidBody")
...
func _set_linear_velocity(node: Node, args: Dictionary) -> Dictionary:
	...
	var y = args.get("y", 0.0)
	var z = args.get("z", 0.0)
	if node is RigidBody2D: node.linear_velocity = Vector2(x, y)
	elif node is RigidBody3D: node.linear_velocity = Vector3(x, y, z)
	elif node is CharacterBody2D: node.velocity = Vector2(x, y)
	elif node is CharacterBody3D: node.velocity = Vector3(x, y, z)
	else: return _error("Node does not support velocity")
...
func _set_angular_velocity(node: Node, args: Dictionary) -> Dictionary:
	...
	if node is RigidBody2D: node.angular_velocity = value
	elif node is RigidBody3D: node.angular_velocity = Vector3(args.get("x", 0.0), args.get("y", 0.0), args.get("z", 0.0))
	else: return _error("Node is not a RigidBody")
