...
var subprocess_manager: SubprocessManager
var dock: Control
...
func _enter_tree() -> void:
	...
	subprocess_manager = (load("res://addons/godot_mcp/subprocess_manager.gd") as GDScript).new()
	subprocess_manager.set_debug_mode(settings.debug_mode)
	add_child(subprocess_manager)

	mcp_server = (load("res://addons/godot_mcp/mcp_server.gd") as GDScript).new()
	...
	mcp_server.set_subprocess_manager(subprocess_manager)
	mcp_server.set_editor_plugin(self)
	...
	print("[Godot MCP] Plugin loaded (hybrid version)")
...
func _create_server_tab() -> Control:
	...
	hybrid_label.text = "Hybrid: Coding-Solo + DaxianLee + tomyud1"
	hybrid_label.add_theme_color_override("font_color", Color(0.6, 0.6, 0.6))
	author_info.add_child(hybrid_label)

	var usage_title = Label.new()
	usage_title.text = "使用说明"
	usage_title.add_theme_color_override("font_color", Color(0.4, 0.7, 1.0))
	author_info.add_child(usage_title)

	var usage_steps = VBoxContainer.new()
	usage_steps.add_theme_constant_override("separation", int(_scaled(4)))
	author_info.add_child(usage_steps)

	var step1 = Label.new()
	step1.text = "1. 点击 IDE 对应的一键配置按钮"
	step1.add_theme_color_override("font_color", Color(0.6, 0.6, 0.6))
	usage_steps.add_child(step1)

	var step2 = Label.new()
	step2.text = "2. 等待配置成功提示"
	step2.add_theme_color_override("font_color", Color(0.6, 0.6, 0.6))
	usage_steps.add_child(step2)

	var step3 = Label.new()
	step3.text = "3. 重启 IDE 使配置生效"
	step3.add_theme_color_override("font_color", Color(0.6, 0.6, 0.6))
	usage_steps.add_child(step3)

	var step4 = Label.new()
	step4.text = "4. 在 IDE 的 MCP 设置中确认 godot-mcp 已连接"
	step4.add_theme_color_override("font_color", Color(0.6, 0.6, 0.6))
	usage_steps.add_child(step4)
...
func _create_config_tab() -> Control:
	...
	content.add_child(_create_config_section("Trae", _get_trae_config_path(), _get_mcp_config(), "trae"))
	content.add_child(_create_config_section("Cursor", _get_cursor_config_path(), _get_mcp_config(), "cursor"))
	content.add_child(_create_config_section("Windsurf", _get_windsurf_config_path(), _get_windsurf_config(), "windsurf"))
...
func _create_category_section(category: String, tools: Array) -> VBoxContainer:
	...
	toggle_btn.text = ">" if is_collapsed else "v"
...
func _get_cursor_config_path() -> String:
	match OS.get_name():
		"Windows": return OS.get_environment("APPDATA") + "/Cursor/User/mcp.json"
		_: return _get_home_dir() + "/.cursor/mcp.json"


func _get_windsurf_config_path() -> String:
	match OS.get_name():
		"Windows": return OS.get_environment("APPDATA") + "/Codeium/windsurf/mcp_config.json"
		"macOS": return _get_home_dir() + "/Library/Application Support/Codeium/windsurf/mcp_config.json"
		_: return _get_home_dir() + "/.codeium/windsurf/mcp_config.json"


func _get_trae_config_path() -> String:
	var home = _get_home_dir()
	match OS.get_name():
		"macOS": return home + "/Library/Application Support/Trae CN/User/mcp.json"
		"Windows": return OS.get_environment("APPDATA") + "/Trae CN/User/mcp.json"
		_: return home + "/.config/Trae CN/User/mcp.json"
...
func _on_cli_scope_changed(index: int) -> void:
	_current_cli_scope = "user" if index == 0 else "project"
...
func _write_config_file(config_type: String, filepath: String, new_config: String, client_name: String) -> void:
	if not FileAccess.file_exists(filepath):
		...
		if not DirAccess.dir_exists_absolute(dir_path):
			...
			dialog.dialog_text = "目录不存在，将自动创建目录：\n%s\n是否继续？" % dir_path
			dialog.title = "Godot MCP"
			if dock:
				dock.add_child(dialog)
				dialog.popup_centered()
				dialog.confirmed.connect(func():
					_do_write_config_file(config_type, filepath, new_config, client_name)
				)
...
func _do_write_config_file(config_type: String, filepath: String, new_config: String, client_name: String) -> void:
	var json = JSON.new()
...
func _on_debug_toggled(pressed: bool) -> void:
	...
	if subprocess_manager:
		subprocess_manager.set_debug_mode(pressed)
	_save_settings()
